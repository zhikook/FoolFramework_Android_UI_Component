...
import zy.fool.internal.InnerBaseWindow.State;
...
public final class OverscrollHelper {
	 ...
	 */
	public static void overScrollBy(final InnerBaseWindow<?> view, final int deltaX, final int scrollX,
	...
	public static void overScrollBy(final InnerBaseWindow<?> view, final int deltaX, final int scrollX,
		...
		//switch (view.getSlideToRefreshScrollDirection()) {
			//case HORIZONTAL:
		deltaValue = deltaX;
		scrollValue = scrollX;
		...
		// refreshing.
		if (view.isSlideToRefreshOverScrollEnabled() && !view.isRefreshing()) {
			final Mode mode = view.getMode();

			// Check that Pull-to-Refresh is enabled, and the event isn't from
			// touch
			if (mode.permitsSlideToRefresh() && !isTouchEvent && deltaValue != 0) {
...
//				}
				...
				if (newScrollValue < (0 - fuzzyThreshold)) {
					...
					// then move scroll
					if (mode.showLefterLoadingLayout()) {
						...
						view.setLefterScroll((int) (scaleFactor * (currentScrollValue + newScrollValue)));
					}
				} else if (newScrollValue > (scrollRange + fuzzyThreshold)) {
					// Check the mode supports the overscroll direction, and
					// then move scroll
					if (mode.showRighterLoadingLayout()) {
						// If we're currently at zero, we're about to start
						// overscrolling, so change the state
						if (currentScrollValue == 0) {
							view.setState(State.OVERSCROLLING);
						...
						view.setLefterScroll((int) (scaleFactor * (currentScrollValue + newScrollValue - scrollRange)));
					}
